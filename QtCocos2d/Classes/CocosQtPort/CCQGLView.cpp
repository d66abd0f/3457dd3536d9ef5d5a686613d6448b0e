...
bool CCQGLView::Create()
{
    ...
    do
    {
        ...
        _window = new CCQGLWidget(_viewSize.width, _viewSize.height);
        ...
        _window->setBaseSize(_viewSize.width, _viewSize.height);
		_window->setMinimumSize(_viewSize.width, _viewSize.height);
		_window->setMaximumSize(_viewSize.width, _viewSize.height);
        _window->show();

		setDesignResolutionSize(_viewSize.width, _viewSize.height, ResolutionPolicy::SHOW_ALL);
		ret = initGlew();
        CC_BREAK_IF(!ret);
